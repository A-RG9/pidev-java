...
public class DailyPlanController {
    ...
    private void setupUI() {
        ...
        // Style du conteneur d'exercices
    ...
    private void setupFilters() {
        if (filterStatusPlan != null) {
            ...
            filterStatusPlan.valueProperty().addListener((obs, old, val) -> loadPlansList());
        }

        if (filterDate != null) {
            filterDate.valueProperty().addListener((obs, old, val) -> loadPlansList());
        }
    ...
    private void loadPlansList() {
        ...
        List<DailyPlan> filteredPlans = plans.stream()
        ...
        if (filteredPlans.isEmpty()) {
            showEmptyState();
            if (lblResultCount != null) {
                lblResultCount.setText("0 plan trouvé");
            }
            updateStatistics();
            return;
        }

        for (DailyPlan plan : filteredPlans) {
        ...
        if (lblResultCount != null) {
            lblResultCount.setText(filteredPlans.size() + " plan(s) trouvé(s)");
        }
    ...
    // Méthodes corrigées sans Toast (utilisation d'Alertes simples)
    ...
    public void backToPlanner(ActionEvent event) {
        try {
            Parent root = FXMLLoader.load(getClass().getResource("/org/example/views/WorkoutPlanner.fxml"));
