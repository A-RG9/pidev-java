...
public class DashboardController extends BaseController {
    ...
    public void initialize() {
        ...
        try {
            ...
            showCustomAlert(Alert.AlertType.ERROR, "Database Connection Error",
                    "The Dashboard could not retrieve data from the database.\nDetails: " + e.getMessage());
        }
    }
    ...
    private void refreshData() {
        try {
            ...
            // Set a placeholder message if the database is connected but empty
            if (logs.isEmpty()) {
                tableRepas.setPlaceholder(new Label("Aucun repas trouvé pour aujourd'hui.\nAjoutez-en un !"));
            }

        } catch (Exception e) {
            System.err.println("Error refreshing data: " + e.getMessage());
            tableRepas.setPlaceholder(new Label("Erreur de connexion à la base de données."));
        }
    }
    ...
    private void loadDailyGoal() {
        try {
            ...
            lblDailyGoalName.setText("Erreur de chargement de l'objectif");
        }
    }
    ...
    private void openFoodDialog(FoodLog existingLog) {
        ...
        dialog.showAndWait().ifPresent(log -> {
            ...
            if (success) {
                ...
                showCustomAlert(Alert.AlertType.ERROR, "Erreur d'insertion",
                        "Impossible de sauvegarder le repas. Vérifiez que la base de données est connectée " +
                                "et que l'utilisateur avec l'UUID existe.");
            }
        });
    }
