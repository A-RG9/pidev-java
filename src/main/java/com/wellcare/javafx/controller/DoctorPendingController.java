...
import com.wellcare.javafx.service.AppointmentEmailService;
...
public class DoctorPendingController {
    ...
    private VBox createAppointmentCard(Consultation apt) {
        ...
        addDetailRow(detailsGrid, 0, "ðŸ“… Date:", apt.getDateConsultation() != null ? apt.getDateConsultation().format(dateFmt) : "N/A");
        addDetailRow(detailsGrid, 1, "â±ï¸ Heure:", apt.getTimeConsultation() instanceof LocalTime ? apt.getTimeConsultation().format(timeFmt) : "N/A");
        addDetailRow(detailsGrid, 2, "â³ DurÃ©e:", apt.getDuration() != null ? apt.getDuration() + " min" : "N/A");
        addDetailRow(detailsGrid, 3, "ðŸ·ï¸ Type:", apt.getConsultationType() != null ? apt.getConsultationType() : "Non spÃ©cifiÃ©");
        addDetailRow(detailsGrid, 4, "ðŸŽ¥ Mode:", apt.getAppointmentMode() != null ? apt.getAppointmentMode() : "PrÃ©sentiel");
        addDetailRow(detailsGrid, 5, "ðŸ“ Lieu:", apt.getLocation() != null ? apt.getLocation() : "Cabinet");

        VBox reasonBox = new VBox(6);
        reasonBox.setStyle("-fx-background-color: #f9fafb; -fx-padding: 12; -fx-background-radius: 8;");
        Label reasonTitle = new Label("ðŸ“ Motif:");
        reasonTitle.setStyle("-fx-font-weight: bold; -fx-font-size: 13px;");
        Label reasonText = new Label(apt.getReasonForVisit() != null ? apt.getReasonForVisit() : "Non spÃ©cifiÃ©");
        ...
        if (apt.getSymptomsDescription() != null && !apt.getSymptomsDescription().isBlank()) {
            Label symptomsTitle = new Label("ðŸŒ¡ï¸ SymptÃ´mes:");
        ...
        Button acceptBtn = new Button("âœ… Accepter");
        ...
        Button rejectBtn = new Button("âŒ Refuser");
        ...
        Label requestedLabel = new Label("DemandÃ© le " + createdAt);
    ...
    private void acceptAppointment(Consultation apt, Button button) {
        ...
        confirm.setContentText("Il sera ajoutÃ© Ã  votre planning et le patient sera notifiÃ©.");
        if (confirm.showAndWait().orElse(ButtonType.CANCEL) != ButtonType.OK) return;

        button.setText("â³ Acceptation...");
    ...
    private void rejectAppointment(Consultation apt, Button button) {
        ...
        new Thread(() -> {
            try {
                ...
                Platform.runLater(() -> {
                    removeCardWithAnimation(apt.getId());
                    showToast("Rendez-vous refusÃ©", Color.RED);
                    if (pendingAppointments.isEmpty()) updateUI();
                });
            } catch (SQLException e) {
                Platform.runLater(() -> {
                    button.setText("âŒ Refuser");
                    button.setDisable(false);
    ...
    private void sendEmailNotification(Consultation apt) {
        try {
            // Static email for testing â€“ bypass database lookup
            ...
            AppointmentEmailService.sendAppointmentAcceptedEmail(
