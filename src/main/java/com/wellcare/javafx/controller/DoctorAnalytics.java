...
public class DoctorAnalytics {
    ...
    private void loadMockData() {
        ...
        planningSuggestionsList.setItems(FXCollections.observableArrayList(
                "RÃ©duire les creux\nVous avez des crÃ©neaux vides importants. Regroupez les consultations ou ouvrez des crÃ©neaux ciblÃ©s.",
                "RÃ©partir la charge\nCertaines journÃ©es sont surchargÃ©es. RÃ©partissez les rendez-vous sur la semaine.",
                "Ajouter des marges\nLes consultations sont trÃ¨s rapprochÃ©es. Ajoutez des marges pour Ã©viter les retards."
        ));

        recommendationsList.setItems(FXCollections.observableArrayList(
                "Activez les rappels automatiques pour rÃ©duire les absences",
                "Augmentez le nombre de suivis pour amÃ©liorer l'observance",
                "Utilisez l'IA pour prioriser les patients Ã  risque",
                "Proposez des forfaits de suivi pour fidÃ©liser"
        ));
    ...
    private Map<String, Object> generateMockProfit() {
        ...
        profit.put("alert_message", "BAISSE DE REVENUS PRÃ‰VUE\nLes revenus prÃ©vus sont infÃ©rieurs Ã  80% des 30 derniers jours.");
        return profit;
    }
    ...
    private void displayPredictions(List<Map<String, Object>> predictions) {
        ...
        for (Map<String, Object> p : predictions) {
            ...
            // Manual date formatting â€“ NO DateTimeFormatter used
    ...
    @FXML private void generateQuickReport() { showAlert("Rapport", "FonctionnalitÃ© Ã  implÃ©menter"); }
    private void generateQuickReport(Patient patient) { showAlert("Rapport", "GÃ©nÃ©ration du rapport pour " + patient.getName()); }
    private void viewPatientDetails(Patient patient) { showAlert("DÃ©tails", "Patient: " + patient.getName() + "\nÃ‚ge: " + patient.getAge() + "\nScore santÃ©: " + patient.getHealthScore()); }
    @FXML private void closePatientModal() { patientModal.setVisible(false); }
    private void sendMessageToPatient(Patient patient) { showAlert("Message", "Envoi d'un message Ã  " + patient.getName()); }
