...
import com.wellcare.javafx.model.Examens;
import com.wellcare.javafx.service.ConsulationServices;
import com.wellcare.javafx.service.OrdonnanceServices;
import com.wellcare.javafx.service.ExamensServices;
...
public class ClinicalNotesController {
    ...
    private void initialize() {
        try {
            ...
            if (editorMeta != null) editorMeta.setText("Date: " + LocalDate.now() + " â€¢ Dr. Name");

            if (followUpType != null && followUpType.getItems() != null) {
                if (followUpType.getItems().isEmpty()) {
                    followUpType.getItems().addAll("Consultation", "ContrÃ´le");
                    followUpType.setValue("ContrÃ´le");
                }
            }
    ...
    private void loadConsultation(Consultation consultation) {
        ...
        saveBtnText.setText("Mettre Ã  jour");
    }
    ...
    private VBox createMedicationCard() {
        ...
        nameField.setPromptText("Nom du mÃ©dicament");
        ...
        form.add(new Label("FrÃ©quence"), 0, 2);
        form.add(frequencyField, 1, 2);
        form.add(new Label("DurÃ©e"), 0, 3);
    ...
    private VBox createExamCard() {
        ...
        form.add(new Label("DÃ©tails"), 0, 1);
    ...
    private void saveNoteToServer() {
        if (subjective == null || objective == null || assessment == null || plan == null) {
            showError("Erreur", "Les champs du formulaire ne sont pas initialisÃ©s.");
            return;
        }
        try {
            ...
            if (currentConsultationId > 0) {
                ...
                showInfo("SuccÃ¨s", "Note mise Ã  jour avec succÃ¨s!");
            } else {
                ...
                showInfo("SuccÃ¨s", "Note enregistrÃ©e avec succÃ¨s!");
                loadNotesHistory();
                resetForm();
            }
            ...
            showError("Erreur de format", "VÃ©rifiez les valeurs des constantes.");
        } catch (Exception e) {
    ...
    private void generateAIReport() {
        ...
        aiTask.setOnFailed(event -> {
            root.getChildren().remove(progress);
            showError("Erreur IA", "Ã‰chec de la gÃ©nÃ©ration : " + aiTask.getException().getMessage());
        });
    ...
    private String buildClinicalNoteText() {
        ...
        sb.append("Vitals: BP ").append(bpSystolic != null ? bpSystolic.getText() : "").append("/")
                ...
                .append(temperature != null ? temperature.getText() : "").append(" Â°C, SpO2 ")
        ...
        if (medicationsContainer != null) {
            for (javafx.scene.Node card : medicationsContainer.getChildren()) {
                sb.append("  â€¢ ").append(extractMedicationText(card)).append("\n");
            }
        }
        sb.append("Exams:\n");
        if (examsContainer != null) {
            for (javafx.scene.Node card : examsContainer.getChildren()) {
                sb.append("  â€¢ ").append(extractExamText(card)).append("\n");
            }
        }
    ...
    private String extractMedicationText(javafx.scene.Node card) {
        ...
        return "MÃ©dicament non renseignÃ©";
    }
    ...
    private String findFirstTextField(VBox parent) {
        for (javafx.scene.Node node : parent.getChildren()) {
            if (node instanceof TextField) {
                ...
                return text.isEmpty() ? "Non spÃ©cifiÃ©" : text;
            } else if (node instanceof VBox) {
                String found = findFirstTextField((VBox) node);
                if (!found.equals("Non spÃ©cifiÃ©")) return found;
                ...
                for (javafx.scene.Node inner : hbox.getChildren()) {
                    if (inner instanceof TextField) {
                        ...
                        if (!found2.equals("Non spÃ©cifiÃ©")) return found2;
                    }
                }
    ...
    private String buildAIPrompt(String rawNote) {
        return """
            # Objective
            You are an expert medical report writer. Transform the following raw clinical SOAP notes into a professional, wellâ€‘structured medical report.
            ...
            - Organise the final report under these headings:
                ...
                **Plan** (from PLAN, including medications, exams, followâ€‘up)
    ...
    private String callMistralAPI(String prompt) throws Exception {
        ...
        // Manual JSON building â€“ no external libraries
        ...
        if (response.statusCode() == 200) {
            // Manual JSON parsing â€“ extract content after "content":"
    ...
    private void showReportDialog(String reportText) {
        ...
        dialog.setTitle("Rapport MÃ©dical GÃ©nÃ©rÃ© par IA");
    ...
    private void saveReportAsPDF(String reportText) {
        ...
        if (file != null) {
            try (java.io.FileOutputStream fos = new java.io.FileOutputStream(file)) {
                ...
                document.close();
                showInfo("SuccÃ¨s", "Rapport PDF enregistrÃ© !");
            } catch (Exception ex) {
