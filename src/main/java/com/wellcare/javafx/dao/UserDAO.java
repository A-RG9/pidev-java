...
public class UserDAO {
    ...
    public User createUser(User user, String hashedPassword) throws SQLException {
        String query = "INSERT INTO users (uuid, email, password, first_name, last_name, phone, address, role, license_number, specialite, birthdate, is_active, is_email_verified, is_verified_by_admin, created_at, updated_at, login_attempts, years_of_experience, diploma_url, rating, is_two_factor_enabled) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 0.00, 0)";
        try (PreparedStatement stmt = connection.prepareStatement(query)) {
