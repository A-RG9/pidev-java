package com.wellcare.dao;

import com.wellcare.models.DailyPlan;
import com.wellcare.models.Exercise;
import com.wellcare.utils.Database;
...
public class DailyPlanDAO {
    ...
    public boolean updateDailyPlanGoalId(int planId, int goalId) {
        String sql = "UPDATE daily_plan SET goal_id = ? WHERE id = ?";
        try (Connection conn = Database.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
    ...
    // Dans DailyPlanDAO.java, ajoutez cette méthode :

    public boolean updateDailyPlanStatus(int planId, String status) {
        String sql = "UPDATE daily_plan SET status = ? WHERE id = ?";
